...
bool js_cocos2dx_ui_RichText_getNewContentSizeHeight(JSContext *cx, uint32_t argc, jsval *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    JS::RootedObject obj(cx, args.thisv().toObjectOrNull());
    js_proxy_t *proxy = jsb_get_js_proxy(obj);
    cocos2d::ui::RichText* cobj = (cocos2d::ui::RichText *)(proxy ? proxy->ptr : NULL);
    JSB_PRECONDITION2( cobj, cx, false, "js_cocos2dx_ui_RichText_getNewContentSizeHeight : Invalid Native Object");
    if (argc == 0) {
        double ret = cobj->getNewContentSizeHeight();
        jsval jsret = JSVAL_NULL;
        jsret = DOUBLE_TO_JSVAL(ret);
        args.rval().set(jsret);
        return true;
    ...
    JS_ReportError(cx, "js_cocos2dx_ui_RichText_getNewContentSizeHeight : wrong number of arguments: %d, was expecting %d", argc, 0);
    return false;
}
...
void js_register_cocos2dx_ui_RichText(JSContext *cx, JS::HandleObject global) {
    ...
    static JSFunctionSpec funcs[] = {
        ...
        JS_FN("getNewContentSizeHeight", js_cocos2dx_ui_RichText_getNewContentSizeHeight, 0, JSPROP_PERMANENT | JSPROP_ENUMERATE),
